...
public class Game1 : Game
{
    ...
    private void StartGame(int seed, Difficulty difficulty)
    {
        ...
        _state.ApplyDifficulty(difficulty);
    ...
    private void LoadGame()
    {
        ...
        // restore player-placed walls
        foreach (var w in save.PlayerWalls)
            if (!_map.PlayerPlacedWalls.Contains(new Point(w[0], w[1])))
                _map.PlayerPlacedWalls.Add(new Point(w[0], w[1]));
        ...
        foreach (var tr in save.Towers)
        {
            ...
            {
                if (_map.CanPlace2x2Tower(tr.Col, tr.Row))
                    _map.Place2x2Tower(tr.Col, tr.Row);
            }
            else
                _map.PlaceTower(tr.Col, tr.Row);

            var tower = new Tower(tr.Col, tr.Row, type);
            for (int lvl = 1; lvl < tr.Level; lvl++) tower.Upgrade();
            tower.TowerHealth = tr.TowerHealth;
            tower.AutoRebuildEnabled = tr.AutoRebuildEnabled;
            tower.PlacedDuringPrep = false;
            _towers.Add(tower);
            if (type == TowerType.Repair)
                for (int d = 0; d < tower.DroneCount; d++)
                    _drones.Add(new RepairDrone(tower));
        }

        _waves.SetWave(save.Wave);
        SaveManager.DeleteSave();
    }

    private void SaveGame()
    {
        if (_waves.WaveActive) return; // only save between waves
    ...
    {
        _paused = false; _contextMenu.Close();
        _seeds.NewRandomSeed();
    ...
    private void ProcessAutoRebuild()
    {
        ...
        {
            var rec = _destroyedThisWave[i];
            if (rec.Type == TowerType.Repair) continue;
            Tower? nearest = null; float best = float.MaxValue;
            var wp = Map.GridToWorld(rec.GridPos.X, rec.GridPos.Y);
            foreach (var rt in repairTowers) { float d = Vector2.Distance(rt.WorldPos, wp); if (d < best) { best = d; nearest = rt; } }
            ...
            _towers.Add(nt); _destroyedThisWave.RemoveAt(i);
    ...
    {
        _glossaryReturnScreen = _screen; _screen = GameScreen.Glossary;
        _glossaryIndex = 0; AudioManager.Instance.Play("ui_click", 0.4f);
    }

    private void OpenSettings(GameScreen returnTo)
    {
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        // ESC: only toggle pause when playing — NEVER quit
        if (_screen == GameScreen.Playing && JustPressed(kb, Keys.Escape))
        { _paused = !_paused; _contextMenu.Close(); }
    ...
    private void UpdateTitle(KeyboardState kb, MouseState mouse)
    {
        ...
        { StartGame(_seedInputActive && _seedInput.Length > 0 ? int.Parse(_seedInput) : _seeds.CurrentSeed, _selectedDifficulty); return; }
        ...
        if (lc)
        {
            // clickable buttons on title
            float cx = GameSettings.ScreenWidth / 2f;
            if (ClickedRect(mouse, (int)(cx - 200), 200, 400, 22)) { StartGame(_seedInputActive && _seedInput.Length > 0 ? int.Parse(_seedInput) : _seeds.CurrentSeed, _selectedDifficulty); return; }
    ...
    private void UpdatePlaying(GameTime gameTime, KeyboardState kb, MouseState mouse)
    {
        if (_paused)
        {
            ...
            if (LeftClicked(mouse)) HandlePauseClick(mouse);
            return;
        }
    ...
    private void HandlePauseClick(MouseState mouse)
    {
        ...
        int bw = 160, bh = 32, gap = 8;
        int bx = (int)(cx - bw / 2f);
        int by = (int)(cy + 10);
        // Resume
        if (new Rectangle(bx, by, bw, bh).Contains(mouse.X, mouse.Y))
        { _paused = false; AudioManager.Instance.Play("ui_click", 0.4f); return; }
        by += bh + gap;
        // Save (only between waves)
        if (new Rectangle(bx, by, bw, bh).Contains(mouse.X, mouse.Y) && !_waves.WaveActive)
        { SaveGame(); return; }
        by += bh + gap;
        // Settings
        if (new Rectangle(bx, by, bw, bh).Contains(mouse.X, mouse.Y))
        { OpenSettings(GameScreen.Playing); return; }
        by += bh + gap;
        // Restart
        if (new Rectangle(bx, by, bw, bh).Contains(mouse.X, mouse.Y))
        { StartGame(_seeds.CurrentSeed, _state.Difficulty); return; }
        by += bh + gap;
        // Glossary
        if (new Rectangle(bx, by, bw, bh).Contains(mouse.X, mouse.Y))
        { OpenGlossary(); return; }
    ...
    private void UpdateSettings(KeyboardState kb, MouseState mouse)
    {
        ...
        if (!LeftClicked(mouse)) return;

        float cx = GameSettings.ScreenWidth / 2f;
        int panelX = (int)(cx - 200), panelW = 400;
        int y = 140;

        // Difficulty buttons (only from title)
        ...
        if (canChangeDiff)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                { _selectedDifficulty = (Difficulty)i; AudioManager.Instance.Play("ui_click", 0.4f); }
            }
        }
        y += 50;
        ...
        if (sfxBar.Contains(mouse.X, mouse.Y))
        { AudioManager.Instance.SfxVolume = Math.Clamp((mouse.X - panelX) / (float)panelW, 0f, 1f); AudioManager.Instance.Play("ui_click", 0.3f); }
        y += 40;

        // Music slider
        y += 30;
        var musBar = new Rectangle(panelX, y, panelW, 20);
        if (musBar.Contains(mouse.X, mouse.Y))
    ...
    private void HandleInput(KeyboardState kb, MouseState mouse)
    {
        ...
        if (!_contextMenu.IsOpen)
            foreach (var btn in _toolbar) if (btn.Hotkey.HasValue && JustPressed(kb, btn.Hotkey.Value)) btn.OnClick();
        if (JustPressed(kb, Keys.R)) { StartGame(_seeds.CurrentSeed, _state.Difficulty); return; }
        ...
        foreach (var t in _towers)
        {
            if (t.Is2x2) { int dx = _hoverCell.X - t.GridPos.X, dy = _hoverCell.Y - t.GridPos.Y;
    ...
    private void UpgradeTower(Tower t)
    {
        ...
        int db = t.DroneCount; _state.Money -= t.UpgradeCost; t.Upgrade();
        for (int i = db; i < t.DroneCount; i++) _drones.Add(new RepairDrone(t));
    }
    ...
    private void TryPlaceTower()
    {
        ...
        if (_state.SelectedTower == TowerType.Repair)
        {
            ...
            _state.SpendTower(); _map.Place2x2Tower(_hoverCell.X, _hoverCell.Y);
            ...
            _towers.Add(t); _drones.Add(new RepairDrone(t)); _runStats.RecordTowerBuilt();
        }
        else
        {
    ...
    {
        _drones.Clear();
        foreach (var tw in _towers) { if (tw == t || tw.Type != TowerType.Repair) continue;
    ...
    {
        foreach (var e in _enemies) { e.Update(gt);
        ...
        foreach (var e in _enemies)
        {
            ...
            switch (e.Type)
            {
                case EnemyType.Medic:
                    if (e.AbilityCooldown <= 0f) { foreach (var o in _enemies) { if (!o.IsAlive || o == e || o.Health >= o.MaxHealth) continue;
                ...
                case EnemyType.Hacker:
                    if (e.AbilityCooldown <= 0f) { Tower? n = null; float bd = float.MaxValue;
    ...
    private void CleanUp()
    {
        ...
        for (int i = _enemies.Count - 1; i >= 0; i--)
        {
            ...
            if (!e.IsAlive)
            {
                if (!e.ReachedEnd && e.Health <= 0)
                {
                    ...
                    int total = reward + (int)(reward * grind);
                    _state.EarnReward(total); _runStats.RecordKill(e.Type); _runStats.RecordMoney(total);
        ...
        for (int i = _towers.Count - 1; i >= 0; i--)
        {
            if (_towers[i].IsDestroyed)
            {
                ...
                if (t.Is2x2) _map.Remove2x2Tower(t.GridPos.X, t.GridPos.Y); else _map.RemoveTower(t.GridPos.X, t.GridPos.Y);
                if (t.Type == TowerType.Repair) rebuildDr = true;
                _towers.RemoveAt(i); if (_hoveredTower == t) _hoveredTower = null;
            }
        }
    ...
    private void DrawTitle()
    {
        ...
        {
            DrawCentred("--- High Scores ---", cx, 290, Color.Gold);
            int y = 318;
    ...
    private void DrawSettings()
    {
        ...
        float cx = GameSettings.ScreenWidth / 2f;
        DrawCentred("SETTINGS", cx, 60, Color.CornflowerBlue);
        ...
        Color[] diffColors = { new Color(100, 220, 100), Color.CornflowerBlue, new Color(220, 80, 80) };
        int dbw = 120;
        for (int i = 0; i < 3; i++)
        {
            var r = new Rectangle(panelX + i * (dbw + 10), y, dbw, 30);
            bool sel = (int)_selectedDifficulty == i;
            _spriteBatch.Draw(_sprites.Pixel, r, sel ? diffColors[i] * 0.4f : new Color(40, 40, 60));
            DrawCentred(diffs[i], r.X + r.Width / 2f, r.Y + 6, sel ? diffColors[i] : (canChangeDiff ? Color.Gray : Color.DimGray));
        }
        y += 50;

        // SFX Volume
        _spriteBatch.DrawString(_font, $"SFX Volume: {(int)(AudioManager.Instance.SfxVolume * 100)}%", new Vector2(panelX, y), Color.White);
        y += 30;
        DrawSlider(panelX, y, panelW, 20, AudioManager.Instance.SfxVolume, new Color(100, 180, 255));
        y += 40;

        // Music Volume
        _spriteBatch.DrawString(_font, $"Music Volume: {(int)(AudioManager.Instance.MusicVolume * 100)}%", new Vector2(panelX, y), Color.White);
        y += 30;
        DrawSlider(panelX, y, panelW, 20, AudioManager.Instance.MusicVolume, new Color(180, 100, 255));
        y += 60;

        // Back button
        var backRect = new Rectangle((int)(cx - 60), y, 120, 32);
        _spriteBatch.Draw(_sprites.Pixel, backRect, new Color(60, 60, 80));
        DrawCentred("[ESC] Back", cx, y + 7, Color.LightGray);

        // Difficulty descriptions
        y += 60;
        string desc = _selectedDifficulty switch
        {
            Difficulty.Easy => "Easy: 30 lives, $200 start, enemies have 70% HP / 90% speed, 130% rewards",
    ...
    private void DrawSlider(int x, int y, int w, int h, float value, Color fillColor)
    {
        ...
        // handle
        int hx = x + fillW - 3;
        _spriteBatch.Draw(_sprites.Pixel, new Rectangle(Math.Max(x, hx), y - 2, 6, h + 4), fillColor);
    }
    ...
    private void DrawPauseOverlay()
    {
        ...
        bool canSave = !_waves.WaveActive;
        DrawButton(bx, by, bw, bh, canSave ? "Save Game" : "Save (in wave)", canSave ? new Color(100, 200, 255) : Color.DimGray); by += bh + gap;
